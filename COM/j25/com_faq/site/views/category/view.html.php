...
class FaqViewCategory extends JViewLegacy
{
	...
	function display($tpl = null)
	{
		...
		for ($i = 0, $n = count($items); $i < $n; $i++)
		{
			...
			if ($item->params->get('count_clicks', $params->get('count_clicks')) == 1) {
				$item->link = JRoute::_('index.php?option=com_faq&task=faq.go&id='. $item->id);
			}
			else {
				$item->link = $item->url;
			}
	...
	protected function _prepareDocument()
	{
		...
		if ($menu && ($menu->query['option'] != 'com_faq' || $id != $this->category->id)) {
			$this->params->set('page_subheading', $this->category->title);
			$path = array(array('title' => $this->category->title, 'link' => ''));
			$category = $this->category->getParent();

			while (($menu->query['option'] != 'com_faq' || $id != $category->id) && $category->id > 1)
			{
