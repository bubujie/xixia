...
class JInstallationHelper
{
	...
	function createAdminUser(& $vars)
	{
		...
		$query = "INSERT INTO #__users VALUES (61, '佚名', 'administrator', ".$db->Quote($adminEmail).", ".$db->Quote($cryptpass).", 'Super Administrator', 0, 1, 25, '$installdate', '$nullDate', '', '')";
		$db->setQuery($query);
		if (!$db->query())
		{
		...
		$query = "INSERT INTO #__core_acl_aro VALUES (10,'users','61',0,'Administrator',0)";
		$db->setQuery($query);
		if (!$db->query())
		{
			echo $db->getErrorMsg();
