...
JModelLegacy::addIncludePath(JPATH_SITE . '/components/com_faqs/models', 'FaqsModel');
...
class modFaqsHelper
{
	static function getList($params)
	{
		...
		$model = JModelLegacy::getInstance('Category', 'FaqsModel', array('ignore_request' => true));
		...
		// Access filter
		$access = !JComponentHelper::getParams('com_faqs')->get('show_noauth');
		 ...
		 * faqscategory model but I don't see any models using checked_out filters
		...
		for ($i =0, $count = count($items); $i < $count; $i++) {
			...
			if ($item->params->get('count_clicks', $params->get('count_clicks')) == 1) {
				$item->link	= JRoute::_('index.php?option=com_faqs&task=faq.go&catid='.$item->catslug.'&id='. $item->slug);
			} else {
				$item->link = $item->url;
			}
		}
