...
var customizationIdMessage = '{l s='Customization #' mod='blockcart2' js=1}';
var removingLinkText = '{l s='remove this product from my cart' mod='blockcart2' js=1}';
</script>
{/if}
...
<div id="cart_block" class="block exclusive">
	<h4>
		<a href="{$link->getPageLink("$order_process.php", true)}">{l s='Cart' mod='blockcart2'}</a>
		{if $ajax_allowed}
	...
	<div id="cart_block_summary" class="{if isset($colapseExpandStatus) && $colapseExpandStatus eq 'expanded' || !$ajax_allowed || !isset($colapseExpandStatus)}collapsed{else}expanded{/if}">
		...
		<span class="ajax_cart_product_txt_s" {if $cart_qties <= 1}style="display:none"{/if}>{l s='products' mod='blockcart2'}</span>
		<span class="ajax_cart_product_txt" {if $cart_qties > 1}style="display:none"{/if}>{l s='product' mod='blockcart2'}</span>
		...
		<span class="ajax_cart_no_product" {if $cart_qties != 0}style="display:none"{/if}>{l s='(empty)' mod='blockcart2'}</span>
	</div>
				...
				<ul class="cart_block_customizations" id="customization_{$productId}_{$productAttributeId}">
					{foreach from=$customizedDatas.$productId.$productAttributeId key='id_customization' item='customization' name='customizations'}
						<li name="customization">
							...
							{else}
							{l s='Customization #' mod='blockcart2'}{$id_customization|intval}{l s=':' mod='blockcart2'}
							{/if}
						</li>
					{/foreach}
				</ul>
	...
	{/if}
		<p {if $products}class="hidden"{/if} id="cart_block_no_products">{l s='No products' mod='blockcart2'}</p>
		...
		<p id="cart-prices">
			...
			{if $show_tax && isset($tax_cost)}
				<span>{l s='Tax' mod='blockcart2'}</span>
				<span id="cart_block_tax_cost" class="price ajax_cart_tax_cost">{$tax_cost}</span>
				<br/>
			{/if}
			<span>{l s='Total' mod='blockcart2'}</span>
			<span id="cart_block_total" class="price ajax_block_cart_total">{$total}</span>
		</p>
		{if $use_taxes && $display_tax_label == 1 && $show_tax}
			{if $priceDisplay == 0}
				<p id="cart-price-precisions">
					{l s='Prices are tax included' mod='blockcart2'}
				</p>
			{/if}
			{if $priceDisplay == 1}
				<p id="cart-price-precisions">
					{l s='Prices are tax excluded' mod='blockcart2'}
				</p>
			{/if}
		{/if}
		<p id="cart-buttons">
			{if $order_process == 'order'}<a href="{$link->getPageLink("$order_process.php", true)}" class="button_small" title="{l s='Cart' mod='blockcart2'}">{l s='Cart' mod='blockcart2'}</a>{/if}
			<!--<a href="{$link->getPageLink("$order_process.php", true)}{if $order_process == 'order'}?step=1{/if}" id="button_order_cart" class="exclusive{if $order_process == 'order-opc'}_large{/if}" title="{l s='Check out' mod='blockcart2'}"><span></span>{l s='Check out' mod='blockcart2'}</a>-->
			<a href="{$link->getPageLink("$order_process.php", true)}" id="button_order_cart" class="exclusive{if $order_process == 'order-opc'}_large{/if}" title="{l s='Check out' mod='blockcart2'}"><span></span>{l s='Check out' mod='blockcart2'}</a>
		</p>
	</div>
	</div>
</div>
